...
#define thermIntPin A14
#define thermExtPin A15
#define ubloxSerial Serial2       // Serial communication lines for the ublox GPS -- PCB pins: Serial5
...
void updateThermistor(){
  ...
  Tinv = A+B*logR+C*logR*logR*logR;
  T = 1/Tinv;
  currentTempC = T-273.15; // converting to celsius
  currentTempF = currentTempC*9/5+32;
...
void updateDataStrings() {
  altitudeFtGPS =  sparkFunGNSS.getAltitude() * pow(10.0, -3) * 3.280839895; //ublox.getAlt_feet();  //older used in OG pterodactyl
  latitudeGPS = sparkFunGNSS.getLatitude() * pow(10.0, -7); // ublox.getLat();
  longitudeGPS = sparkFunGNSS.getLongitude() * pow(10.0, -7); //ublox.getLon();
  

  groundData = String(millis()) + ", " + String(sparkFunGNSS.getYear()) + "," + String(sparkFunGNSS.getMonth()) + "," + String(sparkFunGNSS.getDay()) + "," +
               String(sparkFunGNSS.getHour() - 5) + "," + String(sparkFunGNSS.getMinute()) + "," + String(sparkFunGNSS.getSecond()) + "," //ability to go to milliseconds (or nano) with sparkfun library
               + String(latitudeGPS, 7) + ", " + String(longitudeGPS, 7) + ", " + String(altitudeFtGPS, 7) + "," 
               + String(latCalc, 7) + ", " + String(longCalc, 7) + ", " + String(altCalc, 7) + ", " + String(altitudeFt) + ", " 
               + String(ecefX) + ", " + String(ecefY) + ", " + String(ecefZ)
               + ", " + String(UTMZoneNum) + String(UTMZoneLetter) + " " + String(UTMEasting) + "mE " + String(UTMNorthing) + "mN"
               + ", " + String(thermistorInt) + ", " + String(thermistorExt) + ", "
               + ", " + String(msTemperature) + ", " + String(0) + "," + String(msPressure) + "," + String((millis() - ppodOffset - 2000) / 1000) + "," ;
...
void updateStatus(){
  ...
  bitWrite(Statusbyte, 2, 0);
  bitWrite(Statusbyte, 1, 0);
  bitWrite(Statusbyte, 0, 0);
}
